...
describe('Graph', () => {
  ...
  describe('addNode', () => {
    it('should add a node to the graph', () => {
      ...
      expect(friends.nodes.hasOwnProperty('David')).toBe(true);
    });
  });
  ...
  describe('addEdge', () => {
    it('should add an edge for two nodes', () => {
      ...
      expect(nycToJc && jcToNyc).toBe(true);
    });
  });
  ...
  describe('removeNode', () => {
    it('should remove a node from the graph', () => {
      ...
      expect(friends.nodes.hasOwnProperty('David')).toBe(false);
    });
  });
  ...
  describe('removeEdge', () => {
    it('should remove an edge for two nodes', () => {
      ...
      const nycToJcDisconnected = !cities.nodes['Los Angeles'].some(
        edge => edge === 'San Francisco'
      );
      const jcToNycDisconnected = !cities.nodes['San Francisco'].some(
        edge => edge === 'Los Angeles'
      );
      const disconnected = nycToJcDisconnected && jcToNycDisconnected;
      expect(connected && disconnected).toBe(true);
    });
  });
  ...
  describe('contains', () => {
    ...
    it('should return false if value is not found', () => {
      expect(nums.contains('4')).toBe(false);
    });
  });
  ...
  describe('pathExists', () => {
    it('should return true if a path exists from one node to another', () => {
      ...
      expect(cities.pathExists('JC', 'Montreal')).toBe(true);
    });
    ...
    it('should return false if no path exists between two nodes', () => {
      ...
      cities.addEdge('NYC', 'Boston');
      expect(cities.pathExists('NYC', 'Honolulu')).toBe(false);
    });
  });
